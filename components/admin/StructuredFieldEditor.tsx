...
function ConditionNodeEditor({
  ...
  return (
    <div className="space-y-2 rounded-md border bg-background p-3" data-testid="condition-node">
...
function FlowEditor({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  ...
  return (
    <div className="space-y-2" data-testid="structured-flow">
      {flow.length === 0 ? <p className="rounded-md border border-dashed p-3 text-xs text-muted-foreground">{t('empty')}</p> : null}
      {flow.map((node, index) => (
        <fieldset key={`flow-${index}`} className="space-y-2 rounded-md border p-3">
          <legend className="px-1 text-xs font-medium">{t('question', { number: index + 1 })}</legend>
          <div className="flex flex-wrap items-end gap-2">
  ...
  return (
    <div className="grid gap-3" data-testid="structured-actions">
      {CLASSIFICATIONS.map((classification) => {
        ...
        return (
          <fieldset key={classification} className="space-y-2 rounded-md border p-3">
            <legend className="px-1 text-xs font-medium">{t(`classification.${classification}`)}</legend>
            {action ? (
              <>
...
function OptionsEditor({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  ...
  return (
    <div className="space-y-2" data-testid="structured-options">
      {options.length === 0 ? <p className="rounded-md border border-dashed p-3 text-xs text-muted-foreground">{t('empty')}</p> : null}
      {options.map((option, index) => (
        <fieldset key={`option-${index}`} className="space-y-2 rounded-md border p-3">
          <legend className="px-1 text-xs font-medium">{t('choice', { number: index + 1 })}</legend>
...
function BranchEditor({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  ...
  if (!branch) {
    return (
      <div className="rounded-md border border-dashed p-3" data-testid="structured-branch">
  ...
  return (
    <div className="space-y-2 rounded-md border p-3" data-testid="structured-branch">
  ...
  return (
    <div className="space-y-3" data-testid="structured-factors">
      ...
      {FACTOR_KEYS.map((key) => {
        ...
        return (
          <fieldset key={key} className="space-y-3 rounded-md border p-3">
            <legend className="px-1 text-sm font-medium">{t(`names.${key}`)}</legend>
            ...
            <div className="space-y-2 rounded-md bg-muted/30 p-3">
              ...
              {factor.mapping.map((mapping, index) => (
                <div key={`mapping-${key}-${index}`} className="space-y-2 rounded-md border bg-background p-3">
  ...
  return (
    <div className="grid gap-2 sm:grid-cols-2" data-testid="structured-thresholds">
      {CLASSIFICATIONS.map((classification) => {
        ...
        return (
          <fieldset key={classification} className="rounded-md border p-3">
            <legend className="px-1 text-xs font-medium">{t(`classification.${classification}`)}</legend>
